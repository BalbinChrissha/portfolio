...
export function Model(props) {
    const { nodes, materials } = useGLTF('/laptop_concept.glb')
    const isLaptopRef = useRef();

    useFrame((state, delta) => {
        //isLaptopRef.current.rotation.x += delta
        isLaptopRef.current.rotation.y -= delta * 0.2
        //ref.current.position.z += Math.sin(state.clock.elapsedTime) * 2

    })

    return (
        <group args={[0, 0, 0]} dispose={null} position={[0, 0, 1]} scale={0.050} ref={isLaptopRef} >
...
// useGLTF.preload('/laptop_concept.glb')
